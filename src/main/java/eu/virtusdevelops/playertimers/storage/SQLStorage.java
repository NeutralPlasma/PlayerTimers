package eu.virtusdevelops.playertimers.storage;
...
public class SQLStorage {
    ...
    public SQLStorage(PlayerTimers plugin) throws InvalidConfigurationException {
        this.plugin = plugin;
    ...
    public Map<UUID, List<PlayerTimer>> getTimers(){
        Map<UUID, List<PlayerTimer>> timers = new HashMap<>();
        try (Connection connection = dataSource.getConnection()) {
            PreparedStatement statement = connection.prepareStatement(
            """
                SELECT *
            ...
            while(resultSet.next()){
                ...
                timer.getCommands().addAll(getTimerCommands(connection, timer.getTimerID()));
    ...
    private void insertTimerCommands(PlayerTimer timer, Connection connection) throws SQLException{
        for(var command : timer.getCommands()){
            ...
            VALUES
                (?, ?)
            """);
            statement.setString(1, timer.getTimerID().toString());
            statement.setString(2, command);
            statement.execute();
        }
    }

    private void removeTimerCommands(PlayerTimer timer, Connection connection) throws SQLException{
        PreparedStatement statement = connection.prepareStatement(
                """
    ...
    public void addTimer(PlayerTimer timer){
        try (Connection connection = dataSource.getConnection()) {
            ...
            statement.setString(3, timer.getoPlayer().getUniqueId().toString());
    ...
    public void updateTimer(PlayerTimer timer, boolean updateCommands){
        try (Connection connection = dataSource.getConnection()) {
            PreparedStatement statement = connection.prepareStatement(
                    """
                        UPDATE pt_timers
            ...
            statement.setString(5, timer.getTimerID().toString());
            statement.execute();
            if(updateCommands){
